...
int spawn_root_child(void) {
  ...
  if (root_child_pid == 0) {
    ...
    // Capture uid BEFORE go-signal: the parent will patch creds,
    // so getuid() returns 0 after go. We need the original 2000/1000.
    uid_t captured_uid_before = (uid_t)getuid();
    ...
    report.uid_before = captured_uid_before;
    errno = 0;
