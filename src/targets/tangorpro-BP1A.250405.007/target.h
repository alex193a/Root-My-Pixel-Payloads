// tangorpro — Pixel Tablet, Android 15
// Build: BP1A.250405.007
// Kernel: 6.1.99-android14-11-gd7dac4b14270-ab12946699  (KMI android14-6.1)
// CI build: 12946699 (kernel_aarch64, clang 17, PGO/BOLT/LTO)
//
// PROVENANCE
// ==========
...
//
//  1. Symbol addresses — the unstripped vmlinux for CI build 12946699
//     (kernel-vmlinux/12946699-vmlinux, llvm-nm, 129943 symbols), whose
//     .text section was verified byte-identical to the kernel Image
//     unpacked (magiskboot) from boot/6199/boot.img (md5 of .text
//     matches; Image header: text_offset=0, magic ARMd). Every symbol
//     referenced here was resolved by name; nothing was pattern-matched
//     or approximated. The vmlinux↔Image mapping is linear:
//     file offset == VA - KIMAGE_TEXT_BASE.
//
//     ⚠ NOTE ON boot/6199: the boot.img originally in that folder was a
//     DIFFERENT kernel (6.1.157-android14-11-gbd23337e42e7-ab14791245,
//     verified via version string) — NOT this build. It was moved aside
//     as boot.img.WRONG-6.1.157-CI14791245 and replaced with the correct
//     boot.img downloaded from ci.android.com
//     (builds/submitted/12946699/kernel_aarch64/latest/raw/boot.img),
//     whose kernel version string and .text match this vmlinux exactly.
//
//  2. Struct member offsets — the kernel's own BTF blob in the vmlinux
//     (CONFIG_DEBUG_INFO_BTF=y, .BTF at file offset 0x163be54, magic
//     0xeb9f). BTF is emitted by pahole from the built vmlinux's DWARF,
//     so it describes *this* build's real layout. (Moot here:
//     CONFIG_RANDSTRUCT_NONE=y in the embedded IKCONFIG.)
//
...
//
// Kernel line 6.1.99 (January 2025). The android14-6.1 KMI is frozen, so
// every struct offset in this header is byte-identical to the other 6.1
// headers (BTF-verified here, not assumed); only the symbol addresses
// moved, and they were all re-extracted from THIS build. This build's
// layout matches the 6.1.75/6.1.124 style: .data at 0x1fc0000, so .data
// symbols carry 0x1fcxxxx-style offsets and the FIND_OFFSET_EN.md
// "digit-drop" heuristic (".data must start with 0x02") does NOT apply
// verbatim (unlike the 2025-era 6.1.145/.157/6.1.162 builds whose .data
// sat at 0x02000000). The ashmem device is a single `ashmem_misc`
// miscdevice (not the `ashmem_miscs[]` array of the later builds).
// (SoC note: tangorpro is gs101, same as bluejay — P0 0x80000000
// inherited from the tested bluejay target; the gs101 vendor_kernel_boot
// DTB declares no static memory@ node, RAM is patched at boot.)
//
...
// is identical to every other android14-6.1 build seen (see note below).

#ifndef OFFSET_H
#define OFFSET_H

#if defined(APP_PAYLOAD) && APP_PAYLOAD
#define BUILD_VARIANT_LABEL "tangorpro-BP1A.250405.007-app"
#else
#define BUILD_VARIANT_LABEL "tangorpro-BP1A.250405.007-root-umh"
#endif
#ifndef BUILD_FINGERPRINT
// Same build train (BP1A.250405.007, Android 15, April 2025) as the other
// repo targets — incremental 13240079 is shared across the devices on
// this build. Confirm with `getprop ro.build.fingerprint` if desired.
#define BUILD_FINGERPRINT "google/tangorpro/tangorpro:15/BP1A.250405.007/13240079:user/release-keys"
#endif
...
// PHYS_OFFSET 0x80000000 inherited from bluejay (same gs101 SoC, tested
// on-device); FIND_OFFSET_EN.md's "gs101: 0x40000000" note is stale for
// the tested boot path. text_offset=0 (Image header) ⇒ kernel loaded at
// 0x80000000.
...
// ── Kernel symbol offsets (llvm-nm on vmlinux 12946699, by name) ────────
#define ASHMEM_IOCTL_OFF            0x00c23908ULL
#define ASHMEM_MMAP_OFF             0x00c24248ULL
#define ASHMEM_OPEN_OFF             0x00c24468ULL
#define ASHMEM_RELEASE_OFF          0x00c244f0ULL
#define ASHMEM_SHOW_FDINFO_OFF      0x00c24610ULL
#define ASHMEM_FOPS_OFF             0x012599e8ULL

// &ashmem_misc.fops = ashmem_misc + offsetof(struct miscdevice, fops) =
// 0x10 (BTF). On this build the ashmem device is a SINGLE miscdevice
// named `ashmem_misc` (0x50 bytes = one struct miscdevice), not
// `ashmem_miscs[]` as on the 2025-era builds. Verified empirically: the
// pointer at ashmem_misc+0x10 == &ashmem_fops, minor 255.
//
...
#define ASHMEM_MISC_FOPS_OFF        0x0212a938ULL /* ashmem_misc(0x212a928) + 0x10 */
...
#define ASHMEM_COMPAT_IOCTL_OFF     0x00c241f0ULL

#define CONFIGFS_READ_ITER_OFF      0x045ef10ULL
#define CONFIGFS_BIN_WRITE_ITER_OFF 0x045f440ULL
#define NOOP_LLSEEK_OFF             0x0394700ULL
#define INIT_TASK_OFF               0x1fced80ULL
#define ROOT_TASK_GROUP_OFF         0x21b4580ULL
#define SELINUX_BLOB_SIZES_OFF      0x159efc8ULL
#define SECURITY_HOOK_HEADS_OFF     0x159e8b8ULL
#define KMALLOC_CACHES_OFF          0x159e3f8ULL
#define ANON_PIPE_BUF_OPS_OFF       0x10e3ad0ULL
#define CALL_USERMODEHELPER_EXEC_WORK_OFF 0x00d4d38ULL
#define SYSTEM_UNBOUND_WQ_OFF       0x1fbae60ULL
...
#define COPY_SPLICE_READ_OFF        0x03e1cacULL /* generic_file_splice_read */
...
#define SELINUX_ENFORCING_OFF       0x22063f0ULL /* selinux_state + 0x0 */
...
#define SLIDE_NFULNL_LOGGER_OFF     0x1fc29c8ULL
...
// boot). Slot [0][0] is the LOG-type entry, which nothing registers and
// which stays zero — pointing the leak there makes boot_id read 16 zero
// bytes.
//
// Cross-check: nfulnl_logger - loggers[0][1] = 0x1fc29c8 - 0x1fc2918 =
// 0xb0 — the same distance measured on 6.1.75, 6.1.124, 6.1.145, 6.1.157
// and 6.1.162. A consistent array layout across six android14-6.1 builds
// confirms the slot.
#define SLIDE_LOGGERS_0_1_OFF       0x1fc2918ULL /* loggers(0x1fc2910) + 8 */
...
// walking random_table (0x20e6320) in 0x40-byte struct ctl_table strides
...
// Verified empirically on the Image: random_table[4].data ==
// 0xffffffc00a227498 == &sysctl_bootid; uuid entry has .data == NULL.
#define SLIDE_RANDOM_BOOT_ID_DATA_OFF 0x20e6428ULL /* &random_table[4].data */

#define SLIDE_INIT_TASK_OFF         INIT_TASK_OFF
#define SLIDE_ROOT_TASK_GROUP_OFF   ROOT_TASK_GROUP_OFF
#define SLIDE_SYSCTL_BOOTID_OFF     0x2227498ULL
...
//     (confirmed three times: x27=&rt_waiter at function top, x2 is the
//      waiter argument of rt_mutex_wait_proxy_lock — 6.1 arg order
//      (lock, to, waiter) — and x1 the waiter argument of
...
// byte-identical to the 6.1.75/6.1.124/6.1.157/6.1.162 measurements —
// the stack frames are compiler-stable across the whole android14-6.1
// line. With the waiter being 0x58 bytes (11 words), words 3..13 are
// used, which still lands inside in/out/ex (words 0..14).
...
// and shmem_fallocate exist in this kernel (0x0e42b24 / 0x02e...), so
...
// stack frames of this build are identical to the other 6.1 measurements
...
// BTF-confirmed; identical to the other 6.1 targets.
...
// ── struct configfs_buffer (BTF, size 0x80) — identical to the 6.1 ──────
